CLASS zfloppy_conversion_utils DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS binary_tab_to_xstring IMPORTING binary_tab    TYPE ANY TABLE
                                                  !length       TYPE i
                                        RETURNING VALUE(result) TYPE xstring
                                        RAISING   zfloppy_conversion_exception.

    CLASS-METHODS xstring_to_binary_tab IMPORTING xstring           TYPE xsequence
                                        EXPORTING VALUE(binary_tab) TYPE INDEX TABLE
                                        RAISING   zfloppy_conversion_exception.

    CLASS-METHODS text_tab_to_string IMPORTING text_tab      TYPE ANY TABLE
                                               !length       TYPE i
                                     RETURNING VALUE(result) TYPE string
                                     RAISING   zfloppy_conversion_exception.

    CLASS-METHODS string_to_text_tab IMPORTING !string         TYPE csequence
                                     EXPORTING VALUE(text_tab) TYPE INDEX TABLE
                                     RAISING   zfloppy_conversion_exception.

    CLASS-METHODS string_to_xstring IMPORTING !string       TYPE csequence
                                              codepage      TYPE cpcodepage
                                    RETURNING VALUE(result) TYPE xstring
                                    RAISING   zfloppy_conversion_exception.

    CLASS-METHODS xstring_to_string IMPORTING xstring       TYPE xsequence
                                              codepage      TYPE cpcodepage
                                    RETURNING VALUE(result) TYPE string
                                    RAISING   zfloppy_conversion_exception.
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA converter_cache TYPE SORTED TABLE OF converter_cache_line WITH UNIQUE KEY codepage.
ENDCLASS.
...
CLASS zfloppy_conversion_utils IMPLEMENTATION.
  ...
  METHOD string_to_xstring.
    TRY.
        ...
        IF sy-subrc <> 0.
          INSERT VALUE #( codepage = codepage )
        ...
        cache->converter_out->convert( EXPORTING data   = string
  ...
  METHOD xstring_to_string.
    TRY.
        ...
        cache->converter_in->convert( EXPORTING input = xstring
