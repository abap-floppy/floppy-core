CLASS zfloppy_codepage_helper DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS:
      get_codepage_by_external_name IMPORTING external_name TYPE csequence
                                    RETURNING VALUE(result) TYPE cpcodepage
                                    RAISING   zfloppy_codepage_exception,
      get_system_codepage RETURNING VALUE(result) TYPE cpcodepage,
      get_utf8 RETURNING VALUE(result) TYPE cpcodepage
               RAISING   zfloppy_codepage_exception,
      analyze_xstring IMPORTING xstring       TYPE xstring
...
CLASS zfloppy_codepage_helper IMPLEMENTATION.
  METHOD get_codepage_by_external_name.
    ...
    DO 2 TIMES.
      ...
      DATA(return_code) = cl_i18n_utils=>get_cp_from_name(
        EXPORTING
          im_name = external_name
