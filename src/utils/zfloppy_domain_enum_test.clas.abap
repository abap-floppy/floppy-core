CLASS zfloppy_domain_enum_test DEFINITION
  ...
  PUBLIC SECTION.
    METHODS:
      ...
      constructor IMPORTING class                TYPE data
                            domain               TYPE data
                            value_attribute_name TYPE abap_attrname DEFAULT 'VALUE',
      all_attributes_are_in_domain FOR TESTING,
      all_domain_values_are_attrs FOR TESTING,
      values_should_be_unique FOR TESTING.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA:
      class_descriptor     TYPE REF TO cl_abap_classdescr,
      element_descriptor   TYPE REF TO cl_abap_elemdescr,
      domain_fixed_values  TYPE ddfixvalues,
      attributes           TYPE  abap_attrdescr_tab,
      value_attribute_name TYPE abap_attrname.
    METHODS:
      setup,
      teardown.
ENDCLASS.
...
CLASS zfloppy_domain_enum_test IMPLEMENTATION.
  ...
  METHOD setup.
    element_descriptor->get_ddic_fixed_values(
      RECEIVING
        p_fixed_values = domain_fixed_values
      EXCEPTIONS
        not_found      = 1
        no_ddic_type   = 2
        OTHERS         = 3 ).
    ...
    DELETE attributes WHERE NOT (
      is_class = abap_true AND
      type_kind = cl_abap_typedescr=>typekind_oref AND
      visibility = cl_abap_objectdescr=>public AND
