CLASS zfloppy_file_browser_control DEFINITION
  ...
  PUBLIC SECTION.
    ...
    EVENTS:
      ...
      current_directory_changed EXPORTING VALUE(old_directory) TYPE REF TO zfloppy_path
  ...
  PRIVATE SECTION.
    DATA:
      ...
      inner_splitter_top     TYPE REF TO cl_gui_splitter_container,
      toolbar                TYPE REF TO cl_gui_toolbar,
      path_input             TYPE REF TO cl_gui_input_field,
      directory_listing      TYPE REF TO zfloppy_dir_listing_control,
      directory_tree         TYPE REF TO zfloppy_directory_tree_control,
      status_bar             TYPE REF TO zfloppy_status_bar_control,
      actual_display_options TYPE display_options.
    METHODS:
      refresh_ui RAISING cx_salv_error,
      on_path_input_submit FOR EVENT submit OF cl_gui_input_field IMPORTING input,
      on_change_directory_requ_tree FOR EVENT change_directory_requested OF zfloppy_directory_tree_control
        IMPORTING directory,
      on_change_directory_requ_list FOR EVENT change_directory_requested OF zfloppy_dir_listing_control
        IMPORTING directory,
      on_open_file_requested FOR EVENT open_file_requested OF zfloppy_dir_listing_control
        IMPORTING path.
ENDCLASS.
...
*        update_listing( ).
  ...
  METHOD refresh_ui.
    ...
    IF splitter IS NOT BOUND.
      ...
      pixels = cl_gui_cfw=>compute_metric_from_dynp(
          metric = cl_gui_control=>metric_pixel
          x_or_y = 'X'
          in     = 6 ).
      inner_splitter_top->set_column_width(
          id    = 2
          width = pixels ).
      inner_splitter_top->set_column_sash(
          id    = 1
...
*          value = cl_gui_splitter_container=>false ).
      ...
      toolbar = NEW cl_gui_toolbar(
        parent       = inner_splitter_top->get_container(
                          row    = 1
                          column = 2 )
*        shellstyle   =
*        lifetime     =
...
*        name         =
    ...
    IF path_input IS NOT BOUND.
      path_input = NEW cl_gui_input_field(
          parent               = splitter->get_container(
                                     row    = 2
                                     column = 1 )
    ...
    IF status_bar IS NOT BOUND.
      status_bar = NEW #( splitter->get_container(
          row    = 5
          column = 1 ) ).
    ...
    IF directory_tree IS NOT BOUND.
      directory_tree = NEW zfloppy_directory_tree_control(
        parent      = splitter->get_container(
                          row    = 3
                          column = 1 )
        file_system = file_system
