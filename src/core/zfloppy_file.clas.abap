"! <p class="shorttext synchronized" lang="en">File</p>
CLASS zfloppy_file DEFINITION
  ...
  PUBLIC SECTION.
    CLASS-METHODS:
      ...
      for_backend IMPORTING path          TYPE csequence
      ...
      "! @parameter result | Created instance
      "! @raising zfloppy_file_system_exception | Error retrieving the file system instance
      for_frontend IMPORTING path          TYPE csequence
                   RETURNING VALUE(result) TYPE REF TO zfloppy_file
                   RAISING   zfloppy_file_system_exception,
      ...
      "! @parameter result | Created instance
      "! @raising zfloppy_file_system_exception | Error retrieving the file system instance
      for_target_server IMPORTING target_server TYPE REF TO zfloppy_target_server_enum
                                  path          TYPE csequence
                        RETURNING VALUE(result) TYPE REF TO zfloppy_file
                        RAISING   zfloppy_file_system_exception,
      ...
      "! @parameter path | Full path to the file
      "! @parameter result | Created instance
      for_file_system IMPORTING file_system   TYPE REF TO zfloppy_file_system
                                path          TYPE csequence
                      RETURNING VALUE(result) TYPE REF TO zfloppy_file.
    METHODS:
      "! Get the full path to the file
      "! @parameter result | Full path
      get_path RETURNING VALUE(result) TYPE REF TO zfloppy_path,
      "! Check if file exists
      "! @parameter result | File exists
      "! @raising zfloppy_file_system_exception | Error in file system
      exists RETURNING VALUE(result) TYPE abap_bool
             RAISING   zfloppy_file_system_exception,
      "! Create empty file if it doesn not exist
      "! @raising zfloppy_file_system_exception | Error in file system
      touch RAISING zfloppy_file_system_exception,
      ...
      set_current_codepage IMPORTING codepage TYPE cpcodepage
      ...
      get_current_codepage RETURNING VALUE(result) TYPE cpcodepage,
      append_text IMPORTING codepage TYPE cpcodepage OPTIONAL
                            text     TYPE csequence
                  RAISING   zfloppy_file_system_exception
                            zfloppy_codepage_exception,
      get_input_stream,
      get_output_stream,
      read_all_content_as_text IMPORTING codepage      TYPE cpcodepage OPTIONAL
                               RETURNING VALUE(result) TYPE string
                               RAISING   zfloppy_file_system_exception
                                         zfloppy_codepage_exception,
      read_all_content_as_binary RETURNING VALUE(result) TYPE xstring
                                 RAISING   zfloppy_file_system_exception.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA:
      file_system      TYPE REF TO zfloppy_file_system,
      path             TYPE REF TO zfloppy_path,
      current_codepage TYPE cpcodepage.
    METHODS:
      constructor IMPORTING path        TYPE csequence
                            file_system TYPE REF TO zfloppy_file_system,
      determine_codepage_to_use IMPORTING method_parameter_cp TYPE cpcodepage
...
CLASS zfloppy_file IMPLEMENTATION.
  METHOD for_backend.
    result = for_target_server(
        target_server = zfloppy_target_server_enum=>backend
        path          = path ).
  ENDMETHOD.

  METHOD for_frontend.
    result = for_target_server(
        target_server = zfloppy_target_server_enum=>frontend
        path          = path ).
  ENDMETHOD.

  METHOD for_target_server.
    result = for_file_system(
        file_system = zfloppy_file_system_factory=>get_fs_for_target_server( target_server )
        path        = path ).
  ENDMETHOD.
...
*    file_system->append_file_text
    DATA(content) = file_system->read_file_text(
        path     = path->path
...
*    CATCH zfloppy_file_system_exception.
  ...
  METHOD read_all_content_as_text.
    result = file_system->read_file_text(
        path     = path->path
        codepage = determine_codepage_to_use( codepage ) ).
  ENDMETHOD.
ENDCLASS.
