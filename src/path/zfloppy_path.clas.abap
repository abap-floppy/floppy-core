"! <p class="shorttext synchronized" lang="en">Path</p>
"!
...
CLASS zfloppy_path DEFINITION
  ...
  PUBLIC SECTION.
    DATA:
      ...
      path_kind TYPE REF TO zfloppy_path_kind_enum READ-ONLY.
    CLASS-METHODS:
      from_string IMPORTING path          TYPE csequence
      ...
      from_logical_filename IMPORTING logical_filename TYPE fileintern
                            RETURNING VALUE(result)    TYPE REF TO zfloppy_path
                            RAISING   zfloppy_path_exception.
    METHODS:
      append_path IMPORTING path          TYPE csequence
                  RETURNING VALUE(result) TYPE REF TO zfloppy_path
                  RAISING   zfloppy_path_exception,
      is_directory RETURNING VALUE(result) TYPE abap_bool,
      get_extension RETURNING VALUE(result) TYPE string,
      get_filename RETURNING VALUE(result) TYPE string,
      get_base_filename RETURNING VALUE(result) TYPE string,
      is_root_folder RETURNING VALUE(result) TYPE abap_bool,
      get_path_components RETURNING VALUE(result) TYPE string_table.
  PROTECTED SECTION.
  PRIVATE SECTION.
    DATA:
      fs_path TYPE REF TO cl_fs_path.
    CLASS-METHODS:
      "! {@link .data:fs_path} must be provided as mutable (copied) by the caller
      from_fs_path IMPORTING fs_path       TYPE REF TO cl_fs_path
                   RETURNING VALUE(result) TYPE REF TO zfloppy_path
                   RAISING   zfloppy_path_exception,
      parse_path_kind_from_string IMPORTING path          TYPE csequence
  ...
  METHOD from_string.
    TRY.
        DATA(mutable_path_kind) = COND #(
            WHEN path_kind IS BOUND THEN path_kind
            ELSE parse_path_kind_from_string( path ) ).

        result = NEW #( ).
        result->fs_path = cl_fs_path=>create(
            name           = path
