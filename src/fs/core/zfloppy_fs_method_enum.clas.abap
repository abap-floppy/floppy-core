CLASS zfloppy_fs_method_enum DEFINITION
  ...
  PUBLIC SECTION.
    TYPES instances TYPE SORTED TABLE OF REF TO zfloppy_fs_method_enum WITH UNIQUE KEY table_line.
  ...
  PRIVATE SECTION.
    ...
    METHODS constructor IMPORTING method_name TYPE abap_methname.
ENDCLASS.
...
CLASS zfloppy_fs_method_enum IMPLEMENTATION.
  METHOD class_constructor.
    ...
    partial_reader = VALUE #( LET prefix = 'ZFLOPPY_FS_PARTIAL_READER' IN
                              read_file_to_buffer_bin  = NEW #( |{ prefix }~{ 'READ_FILE_TO_BUFFER_BIN' }| )
                              read_file_to_buffer_text = NEW #( |{ prefix }~{ 'READ_FILE_TO_BUFFER_TEXT' }| ) ).

    partial_writer = VALUE #( LET prefix = 'ZFLOPPY_FS_PARTIAL_WRITER' IN
