CLASS zfloppy_frontend_fs_impl DEFINITION
  ...
  PUBLIC SECTION.
    INTERFACES:
      zfloppy_frontend_file_system.
    METHODS:
      constructor IMPORTING frontend_services TYPE REF TO zfloppy_frontend_services.
  PROTECTED SECTION.
  PRIVATE SECTION.
    TYPES:
      x1024 TYPE x LENGTH 1024.
    DATA:
      frontend_services TYPE REF TO zfloppy_frontend_services.
ENDCLASS.
...
CLASS zfloppy_frontend_fs_impl IMPLEMENTATION.
  ...
  METHOD zfloppy_file_system~get_directory_contents.
    TRY.
        frontend_services->directory_list_files(
          EXPORTING
            directory  = path
          IMPORTING
            file_table = result ).
  ...
  METHOD zfloppy_file_system~read_file_bin.
    ...
    TRY.
        ...
        result = zfloppy_conversion_utils=>binary_tab_to_xstring(
          binary_tab = data_tab
          length     = length ).
  ...
  METHOD zfloppy_file_system~read_file_text.
    ...
    TRY.
        ...
        DATA(xstring) = zfloppy_conversion_utils=>binary_tab_to_xstring(
          binary_tab = data_tab
          length     = length ).
        result = zfloppy_conversion_utils=>xstring_to_string(
          xstring  = xstring
          codepage = codepage ).
  ...
  METHOD zfloppy_file_system~write_file_bin.
    ...
    TRY.
        ...
        frontend_services->gui_download(
          bin_filesize = xstrlen( content )
          filename     = path
          filetype     = zfloppy_frontend_services=>file_types-binary
          data_tab     = data_tab ).
  ...
  METHOD zfloppy_file_system~get_supported_methods.
    result = VALUE #(
        ( zfloppy_fs_method_enum=>create_directory )
